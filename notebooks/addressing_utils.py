"""Hexagonal addressing-scheme utilities, ported from HexagDLy's own
notebooks/addressing_utils.py. This is pure numpy/matplotlib geometry with no
PyTorch (or Keras) dependency at all: the addressing scheme that maps a
hexagonal detector layout to a square-tensor index is identical regardless
...
def plot(ax, x, y, grid=False, hexagons=False, lines=False):
    if hexagons:
        patches = [RegularPolygon((x_, y_), 6, radius=1, orientation=-np.pi / 12)
                   for x_, y_ in zip(x, y)]
        collection = PatchCollection(patches, cmap="gist_yarg", alpha=1,
                                      edgecolors="grey", linewidth=.5)
        collection.set_array(np.zeros(len(patches)))
